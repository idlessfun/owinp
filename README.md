...
1. Создай JSON-файл в `app/apps/` (имя — латиницей, например `myapp.json`).
2. Заполни по образцу выше.
3. Положи иконку в `app/resources/icons/<имя>.png`.
4. (Опционально) Положи скриншоты в `app/resources/screenshots/` и укажи их имена в поле `screenshots`.
5. Создай Pull Request — после проверки мейнтейнером программа появится в каталоге у всех пользователей.
...
- [ ] Отдельная админ-программа для управления каталогом (`OWINP Admin`)
- [ ] Раздел «Настройки» в приложении
- [ ] Расширенная кастомизация JSON-карточек
